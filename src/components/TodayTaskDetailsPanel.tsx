...
import { Clock, Target, Lightbulb, CalendarCheck, Sparkles } from 'lucide-react';
...
const formatHowToBullets = (how: string): string[] => {
  if (!how) return [];
  const lines = how.split(/(?:\n|(?<=\.)\s+(?=[A-Z])|(?:^\d+\.\s*))/gm).map(line => line.trim()).filter(line => line.length > 0 && line.length < 100);
  return lines.slice(0, 4);
};
export function TodayTaskDetailsPanel({
  selectedTask,
  dayType
}: TodayTaskDetailsPanelProps) {
  return <motion.div initial={{
    ...
    duration: 0.3
      ...
      <AnimatePresence mode="wait">
        {selectedTask ? <motion.div key={selectedTask.task.title} initial={{
        opacity: 0,
        y: 10
      }} animate={{
        opacity: 1,
        ...
        duration: 0.2
            ...
            <div className="rounded-xl bg-muted/30 p-4">
              <div className="flex items-start gap-2.5">
                ...
                <div className="flex-1">
                  ...
                  {selectedTask.task.explanation?.how ? <ul className="space-y-2">
                      {formatHowToBullets(selectedTask.task.explanation.how).map((bullet, idx) => <li key={idx} className="flex items-start gap-2 text-sm text-foreground/80">
                    ...
                    </ul> : <p className="text-sm text-foreground/80 leading-relaxed">
            ...
            {/* Expected Outcome */}
            {selectedTask.task.explanation?.expected_outcome && <div className="rounded-xl bg-emerald-500/5 border border-emerald-500/10 p-4">
        ...
        opacity: 0
      }} className="rounded-2xl border border-dashed border-border/40 bg-muted/10 p-8 flex flex-col items-center justify-center text-center min-h-[300px] opacity-95">
