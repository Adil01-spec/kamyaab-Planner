...
export function AdvisorViewContent({
  ...
  return (
    <div className="space-y-8 print:space-y-6">
      ...
      {/* 6. Weekly Breakdown - Full Task Details */}
      <section className="print:break-before-auto space-y-4">
        ...
        {plan.weeks?.map((week: any, weekIndex: number) => {
          ...
          return (
            <Card key={weekIndex} className={cn(
              "print:shadow-none print:border-foreground/20",
              ...
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      className={cn(
                        'w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold print:border print:border-foreground/30',
                        isWeekComplete
                          ? 'bg-primary text-primary-foreground print:bg-foreground/10 print:text-foreground'
                  ...
                  {week.tasks?.map((task: any, taskIndex: number) => {
                    ...
                    return (
                      <div
                        key={taskIndex}
                        className={cn(
                          'flex items-center gap-3 p-2 rounded-lg print:border print:border-foreground/10',
                          isCompleted ? 'bg-primary/5 print:bg-transparent' : 'bg-muted/30 print:bg-transparent'
                        )}
                      >
                        <div
                          className={cn(
                            ...
                            isCompleted
                              ? 'border-primary bg-primary print:border-foreground print:bg-foreground'
                              : 'border-muted-foreground/30'
                          )}
                        >
                          {isCompleted && <CheckCircle2 className="w-3 h-3 text-primary-foreground print:text-background" />}
                        </div>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <Badge
                            variant="outline"
                            className={cn(
                              'text-xs print:border-foreground/30 print:text-foreground',
                              task.priority === 'High'
                                ? 'border-destructive/50 text-destructive'
